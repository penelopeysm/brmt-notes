...
I've chosen this dataset since it's the same as the one used in the introduction of https://paulbuerkner.com/software/brms-book/brms-book.pdf.
...
- how the linear predictor µ maps to the likelihood (i.e., the link function)

Obviously, the interface can be fine tuned, but in terms of data structures this is probably our first step.
...
tms = TuringModelSpec(f, poisson_link, (; b_Treatment = Normal(0, 10)))
```
...
The second problem is how do we actually turn the `TuringModelSpec` into a Turing model function.
...
sample(generic, NUTS(), 1000; chain_type=VNChain, progress=false)
```
