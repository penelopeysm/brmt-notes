...
Here `poisson_response(µ)` returns a distribution, but it should be easy enough to generalise the API to allow users to pass e.g. submodels, or just a straight up likelihood function that returns a float.
...
(BTW, we probably want to parallelise that likelihood... and also not specify `resp` as a model parameter, but rather condition on it. But let's ignore that for now.)
...
So that's the first question, i.e., _what_ is the code that we are trying to generate.

The second problem is _how_ do we actually turn the `TuringModelSpec` into that code.
...
(But we're all clever programmers here, so that shouldn't be too much of a problem, right?)
...
Also, judging by the results of the sampling, I'm sure I've messed something up in the implementation.
But also I think this is the inferior approach, so I don't feel like I really care enough to fix it.
...
@model function model_generic(tms::TuringModelSpec, resp, pred)
    ...
    # This is quite ugly, we can't initialise µ = zeros(size(resp))
    # as that breaks ForwardDiff
